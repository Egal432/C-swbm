"""
Enhanced complete analysis with zoomed-in yearly views and site names
"""
...
from visualize_results import (plot_monthly_comparison, plot_scatter_comparison, 
                               plot_timeseries, print_statistics)
...
try:
    ...
    print("⚠️  Enhanced visualizations not available. Using standard plots only.")
    ENHANCED_VIZ_AVAILABLE = False


def extract_site_name(filepath):
    """
    Extract site/country name from filepath.
    
    Examples:
        'Data/Data_swbm_Germany_new.csv' -> 'Germany'
        'spain_data.csv' -> 'Spain'
        'test_data.csv' -> 'Test Site'
    """
    filename = os.path.basename(filepath)
    
    # Try to extract country name
    if 'Germany' in filename:
        return 'Germany'
        ...
        # Generic: remove extensions and common prefixes
        name = filename.replace('.csv', '').replace('Data_swbm_', '').replace('_new', '')
        return name.title()


def run_complete_analysis(filepaths, output_dir='complete_outputs', 
                         exp_runoff=2.0, exp_et=0.5, beta=0.8, whc=150.0,
                         create_plots=True, create_yearly_plots=True, 
                         yearly_focus=None):
    """
    Enhanced complete analysis pipeline with yearly zoomed views.
    ...
    create_yearly_plots : bool
        Generate detailed yearly plots (default: True)
    yearly_focus : int or list of int, optional
        Specific year(s) to plot. If None, plots first complete year.
    ...
    print("="*70)
    print("SIMPLE WATER BALANCE MODEL - ENHANCED ANALYSIS")
    print("="*70)
    print(f"\nModel Parameters:")
    ...
    for idx, filepath in enumerate(filepaths, 1):
        filename = os.path.basename(filepath)
        site_name = extract_site_name(filepath)
        ...
        try:
            ...
            if create_plots:
                ...
                try:
                    ...
                    fig1.suptitle(f'{site_name} - Full Time Series', 
                ...
                try:
                    ...
                    fig2.suptitle(f'{site_name} - Model vs Observations', 
                ...
                try:
                    ...
                    fig3.suptitle(f'{site_name} - Monthly Aggregation', 
                                 fontsize=16, fontweight='bold')
            ...
            # Create enhanced yearly plots
            if create_yearly_plots and ENHANCED_VIZ_AVAILABLE:
                print("  Generating yearly detail plots...")
                
                # Determine which years to plot
                years_in_data = sorted(data['time'].dt.year.unique())
                
                if yearly_focus is not None:
                    # Use specified year(s)
                    if isinstance(yearly_focus, int):
                        years_to_plot = [yearly_focus] if yearly_focus in years_in_data else []
                    else:
                        years_to_plot = [y for y in yearly_focus if y in years_in_data]
                else:
                    # Plot first complete year (has all 365/366 days)
                    years_to_plot = []
                ...
                # Create plots for selected years
                for year in years_to_plot:
                    try:
                        fig = plot_single_year(data, results, year, site_name)
                        if fig is not None:
                            plot_path = os.path.join(output_dir, 
                                                    f"{site_name}_{year}_detailed.png")
                            plt.savefig(plot_path, dpi=300, bbox_inches='tight')
                            plt.close(fig)
                            print(f"    ✓ Saved: {site_name}_{year}_detailed.png")
                    except Exception as e:
                        print(f"    ✗ Yearly plot error for {year}: {e}")
                
                # Create seasonal pattern plot
                try:
                    fig = plot_seasonal_comparison(data, results, site_name)
                    plot_path = os.path.join(output_dir, f"{site_name}_seasonal.png")
                    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
                    plt.close(fig)
                    print(f"    ✓ Saved: {site_name}_seasonal.png")
                except Exception as e:
                    print(f"    ✗ Seasonal plot error: {e}")
            ...
            summary_row = {
                ...
                'file': filename,
                'n_days': len(data),
                'start_date': str(data['time'].min().date()),
                'end_date': str(data['time'].max().date())
            }
...
def quick_comparison_plot(all_results, output_dir='complete_outputs'):
    """Create comparison plots across all sites with site names."""
    if not all_results:
    ...
    axes[0].set_ylabel('Soil Moisture (mm)', fontweight='bold')
    axes[0].set_title('Soil Moisture Comparison Across Sites', fontsize=14, fontweight='bold')
    axes[0].legend(loc='best', fontsize=8)
    axes[0].grid(True, alpha=0.3)
    
    axes[1].set_ylabel('Runoff (mm)', fontweight='bold')
    axes[1].set_title('Runoff Comparison Across Sites', fontsize=14, fontweight='bold')
    axes[1].legend(loc='best', fontsize=8)
    axes[1].grid(True, alpha=0.3)
    
    axes[2].set_ylabel('ET (mm)', fontweight='bold')
    axes[2].set_xlabel('Date', fontweight='bold')
    axes[2].set_title('Evapotranspiration Comparison Across Sites', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    # Run complete analysis with yearly plots
    all_results, summary = run_complete_analysis(
        ...
        whc=150.0,
        create_plots=True,
        create_yearly_plots=True,  # NEW: Enable yearly plots
        yearly_focus=2010  # NEW: Focus on year 2010 (or None for auto)
    )
